...
NDS_PAK="/storage/roms/Emus/rgb30/NDS.pak"
...
if [ "$CURRENT_BINARY" = "./drastic" ]; then
    ...
    for overlay in $FOUND_OVERLAYS; do
        if [ -f "$BG_BILINEAR/$overlay" ]; then
            cp -f "$BG_BILINEAR/$overlay" "$ROMS_ROOT/$overlay"
            chattr +h "$ROMS_ROOT/$overlay" 2>/dev/null
        fi
    done
    ...
    for overlay in $FOUND_OVERLAYS; do
        if [ -f "$BG_NEAREST/$overlay" ]; then
            cp -f "$BG_NEAREST/$overlay" "$ROMS_ROOT/$overlay"
            chattr +h "$ROMS_ROOT/$overlay" 2>/dev/null
        fi
    done
